...
describes what users get. Pick a format with `-o`, or let urbandictionary choose:
a table when writing to a terminal, JSONL when piped.

## Formats

```bash
urbandictionary <command> -o table   # aligned columns for reading
urbandictionary <command> -o jsonl   # one JSON object per line, for piping
urbandictionary <command> -o json    # a single JSON array
urbandictionary <command> -o csv     # spreadsheet friendly
urbandictionary <command> -o tsv     # tab-separated
urbandictionary <command> -o url     # just the URL column
urbandictionary <command> -o raw     # the underlying bytes, unformatted
```
...
```bash
urbandictionary <command> --fields id,title,url
```
...
```bash
urbandictionary <command> --template '{{.URL}} {{.Title}}'
```
...
```bash
urbandictionary <command>            # a table, because this is a terminal
urbandictionary <command> | wc -l    # JSONL, because this is a pipe
```
